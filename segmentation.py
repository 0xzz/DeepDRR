...
class SegmentationNet():
    ...
    def segment(self, input_volume, show_results=True):
        ...
        padded_volume = np.pad(input_volume, [[offset_before[0], offset_after[0]], [offset_before[1], offset_after[1]],
                                              [offset_before[2], offset_after[2]]], mode='edge')

        padded_volume -= mean
        padded_volume /= std

        segmented_volume = np.zeros([3, padded_volume.shape[0], padded_volume.shape[1], padded_volume.shape[2]],
                                    dtype=np.float32)
        ...
        for i in range(0, blocks[0]):
            for j in range(0, blocks[1]):
                for k in range(0, blocks[2]):
                    print(counter)
                    counter += 1
                    curren_block = padded_volume[i * blocksize:(i + 1) * blocksize, j * blocksize:(j + 1) * blocksize,
                    ...
                    presegmentation[2, :, :, :] = (curren_block < (200 - mean) / std) * (
                    ...
                    output_tensor = np.array(
                        self.model.forward(Variable(curren_block_tensor, requires_grad=False)).detach().cpu())
                    segmented_volume[:, i * blocksize:(i + 1) * blocksize, j * blocksize:(j + 1) * blocksize,
                    k * blocksize:(k + 1) * blocksize] = output_tensor[0, :, :, :, :]
        segmented_volume = segmented_volume[:, offset_before[0]:input_volume.shape[0] + offset_before[0],
                           offset_before[1]:input_volume.shape[1] + offset_before[1],
                           offset_before[2]:input_volume.shape[2] + offset_before[2]]
