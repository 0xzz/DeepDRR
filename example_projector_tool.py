...
def generate_projections_on_sphere(CT_volume_path, save_path, min_theta, max_theta, min_phi, max_phi, spacing_theta,
                                   spacing_phi, photon_count, camera, spectrum, scatter=False, origin=[0, 0, 0]):
    # generate angle pairs on a sphere
    thetas, phis = projection_matrix.generate_uniform_angels(min_theta, max_theta, min_phi, max_phi, spacing_theta,
                                                             spacing_phi)
    # generate projection matrices from angles
    proj_mats = projection_matrix.generate_projection_matrices_from_values(camera.source_to_detector_distance,
                                                                           camera.pixel_size, camera.pixel_size,
                                                                           camera.sensor_width, camera.sensor_height,
                                                                           camera.isocenter_distance, phis, thetas)
    ...
    #load CT volume
    trun = 1000
    ori_CT_volume, CT_volume, CT_materials, CT_voxel_size = load_dicom_CT(CT_volume_path, fixed_slice_thinckness=1.0,
    ...
    forward_projections = projector.generate_projections(proj_mats, CT_volume, CT_materials, origin, CT_voxel_size,
                                                         camera.sensor_width, camera.sensor_height, mode="linear",
...
def main():
    #2x2 binning
    camera = Camera(sensor_width=512, sensor_height=512, pixel_size=0.62, source_to_detector_distance=1200,
    ...
    generate_projections_on_sphere(CT_volume_path, save_path, min_theta, max_theta, min_phi, max_phi, spacing_theta,
                                   spacing_phi, photon_count, camera, spectrum, origin=origin, scatter=False)
