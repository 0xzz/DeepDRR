...
class ProjMatrix():
    ...
    def get_conanical_proj_matrix(self, voxel_size, volume_size, origin_shift):
        ...
        source_point[0] = -(
                -0.5 * (volume_size[0] - 1.0) + origin_shift[0] * inv_voxel_scale[0, 0] + inv_voxel_scale[0, 0] *
                camera_ceter[0])
        source_point[1] = -(
                -0.5 * (volume_size[1] - 1.0) + origin_shift[1] * inv_voxel_scale[1, 1] + inv_voxel_scale[1, 1] *
                camera_ceter[1])
        source_point[2] = -(
                -0.5 * (volume_size[2] - 1.0) + origin_shift[2] * inv_voxel_scale[2, 2] + inv_voxel_scale[2, 2] *
                camera_ceter[2])
        return inv_ar, source_point


def generate_projection_matrices_from_values(source_to_detector_distance, pixel_dim_x, pixel_dim_y, sensor_size_x,
                                             sensor_size_y, isocenter_distance, phi_list, theta_list, rho_list=None,
                                             offset_list=None):
...
def generat_rotation_from_angles(phi, theta, rho=0):
    ...
    omc = 1 - cos_t
    R = np.array(
        [[sin_p * sin_p * omc + cos_t, sin_p * neg_cos_p * omc - z * sin_t, sin_p * z * omc + neg_cos_p * sin_t],
         [sin_p * neg_cos_p * omc + z * sin_t, neg_cos_p * neg_cos_p * omc + cos_t,
...
def generate_uniform_angles(min_theta, max_theta, min_phi, max_phi, spacing_theta, spacing_phi):
